...
void lzw_encoding(unsigned char *buff, packet* pptr, semaphores* sems)
{
	...
	for(uint32_t chunk_num = 0; chunk_num < pptr->num_of_chunks; chunk_num++)
	{
		if(pptr->curr_chunk[chunk_num].is_unique)
		{
			...
			makelog(VERB_DEBUG,"Output Code Size: %d \n",output_code_size);
			...
			chunk_header = (bytes_written<<1);
			//std::cout<<"\nLZW Header: "<<chunk_header<<"\n";
			makelog(VERB_DEBUG,"LZW Header: %d \n",chunk_header);
			...
			if(rem_bits){
				...
				makelog(VERB_DEBUG,"No of remaining bits: %d \n",rem_bits);
				//std::cout<<"Remaining Bits:"<<x<<"\n";
				makelog(VERB_DEBUG,"Remaining bits: %d \n",x);
				write_byte = old_byte>>24;
				std::bitset<8> y(write_byte);
				//std::cout<<"Remaining Byte:"<<y<<"\n";
				makelog(VERB_DEBUG,"Remaining byte: %d \n",y);
				memcpy(&file[offset], &write_byte, sizeof(unsigned char));
				offset+= sizeof(unsigned char);
			}
			bit_pack_timer.stop();
